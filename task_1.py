...
def optimize_printing(print_jobs: List[Dict], constraints: Dict) -> Dict:
    ...
    """
    # Перетворюємо список завдань на об'єкти PrintJob
    jobs = [PrintJob(**job) for job in print_jobs]
    
    # Сортуємо завдання за пріоритетом (вищий пріоритет йде першим)
    ...
    for job in jobs:
        ...
        if job.volume > constraints['max_volume']:
            print(f"Обмеження принтера перевищено на {job.id}")
            return {"print_order": print_order, "total_time": total_time}


        # Перевірка, чи можна додати завдання до поточної групи
        if current_volume + job.volume <= constraints['max_volume'] and current_items + 1 <= constraints['max_items']:
            current_batch.append(job)
            current_volume += job.volume
            current_items += 1
        else:
            # Завершення групи, додавання її часу до загального часу
            if current_batch:
                print_order.extend([j.id for j in current_batch])
                total_time += max(j.print_time for j in current_batch)
            # Очищення групи та початок нової
            current_batch = [job]
            current_volume = job.volume
            current_items = 1
